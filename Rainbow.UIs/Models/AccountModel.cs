...
namespace Rainbow.UIs.Models
{
    public class AccountModel
    {
        ...
        /// <returns></returns>
        internal bas_user ValidateUserLogin(string name, string password)
        {
            bas_user sysUser = Bll2bas_user.GetWhere(new { username = name, userpwd = MD5Helper.EncryptString(password) }).ToList().FirstOrDefault();
            if (sysUser != null)
            {
